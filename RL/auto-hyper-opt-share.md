...
阶段二中，从排序结果到用户行为这一步，一个是没有任何确定的模型，比如某个排序一定好某个排序一定不好，现有模型全部基于假设；一个是无法通过回放来探究参数和用户行为之间的关系。因为时间在流逝，用户的状态在发生变化。同一个排序结果，在不同时间给用户，得到的行为数据不同。不同的排序结果，也无法在同一时间给同一个用户获取两个行为数据。因此，这一阶段只能做线上实验，且是近似实验：两组类似用户用不同参数做对比。这大大限制了样本采集的效率。  
...
这里注意区分一下，调参问题和各种pxtr模型训练不太一样的点。pxtr模型试图建立的，是user和item之间的某种联系，可以收集海量user和历史item的交互数据，来建立模型。而调参问题，是试图建立参数和ab指标之间的某种联系，这个数据的上限来自于ab平台并行实验的能力，因此数据更稀疏。  
...
### step1 假设
不管是一阶段还是二阶段，可以统一合并起来，假设是一个十分复杂的函数$f$，输入是参数$\theta$和环境$z$，输出是用户行为指标$a$：  
$`a = f(\theta,z)+\epsilon`$  
其实这相当简化，因为用户行为受多种因素影响，比如此时的天气（晴天看啥都感兴趣）、刚发的工资（心烦看啥都烦）等等，且随着时间推移在变动，每个特征都是一个复杂函数，而要调整的参数只是其中一个。这里为了示意，简化掉其他因素的影响，环境统一用$z$表示。而观测噪声用$\epsilon$表示。  
在这个假设下，一次实验，就得到一个观测数据:  
$a_i = f(\theta_i,z_i)+\epsilon_i$  
![黑盒函数示意](image/blackbox_function.png)
我们假定短时间内环境是不变化的，消掉环境$z$的影响，对$z$取期望，假设：  
...
### step2 训练
现在的问题是，不知道函数$f$具体是什么，以及$f$可能是无法求梯度的。那就可以用各种假设函数来代替，其中一种是神经网络，神经网络能拟合各种复杂函数，且可微。  
假设神经网络为  
$y=g(W,\theta)$  
其中$W$是神经网络的权重参数,$\theta$是神经网络输入，要调的参数。  
可以设置$g(W,\theta)\approx \mathbb{E}_z[f(\theta,z)]$，为代理模型。  
第一步是收集样本，可以在线上不断调整$\theta$，收集指标$a$，得到一系列样本对：  
$D=\{\theta_1,a_1\},\{\theta_2,a_2\}...\{\theta_i,a_i\}$  
用均方误差为损失函数：  
...
然而，这种方法，有这些问题：  
- 之前说的，大量样本难获取，只能获取少量样本
- 少量样本相当于只在一个复杂函数的局部进行了采样，拟合出来的网络只在样本点密集的局部估计准确，其他地方估计偏差会很大
- 再加上，$\theta$的初始值设定，会影响梯度上升能获取到局部最优解。因此，即使经过多次梯度上升，最终也只能找到样本密集点附近的局部最优解。  
- 而且，由于样本点稀疏部位的估计不准，可能给出严重错误的估计。所以，得出最优$\theta$后，还要再去上线做验证。然后再找最优解。  
- 解决样本点稀疏部位估计不准的问题，需要更大量、更均匀的样本点采集。这又回到了最初的问题，成本会很高。
  
> 上次Agent比赛，will问，我就跑多次实验，然后在后台统计各个实验指标，也能找出来一个最优组啊，你的方法和这个比又强在哪了呢？这里一定程度上解释了这个问题，如果成本无限，我可以在$\theta$的值域内挨个试一遍，一定能找到一个最优的$\theta$，但成本不是无限的。
