...
可以纯想象一下，假设黑盒函数是这样的：
![黑盒函数示意](https://raw.githubusercontent.com/YoungAndSure/TechArt/main/RL/image/blackbox_function.png)
...
也就是对L进行梯度下降，  
$W_{i+1} = W_i - \eta_i \nabla L(W_i)$  
![L(W) loss 与梯度下降](https://raw.githubusercontent.com/YoungAndSure/TechArt/main/RL/image/loss_function_gradient_descent.png)
得到让$L(W)$最小的权重参数$W^*$。  
此时就可以得到一个神经网络：  
$y = g(W^*, \theta)$  
...
## 贝叶斯优化  
书接上回，神经网络建了模，但缺点是，当算出一个最优参数时，不能直接表达在未采样区域中的不确定性。  
这是最大化似然方法的通病，只会用已有数据拟合，不会评估可信度，容易过拟合。  
  
贝叶斯优化和神经网络拟合的不同是，贝叶斯是给出函数$f(\theta)$的概率分布为先验，然后输入样本对$\{\theta_i, a_i\}$，得到$f(\theta)$的后验分布。  
![贝叶斯更新示意](https://raw.githubusercontent.com/YoungAndSure/TechArt/main/RL/image/bayesian_update.png)
### 先验分布假设
现在有一系列超参，作为f的输入：  
$$\theta_1, \theta_2, \theta_3...$$
定义kernel，衡量输入之间的相似性，假设kernel选RBF：  
$$k(\theta_i, \theta_j) = \exp(-\frac{||\theta_i-\theta_j||^2}{2l^2})$$
![RBF Kernel 可视化](https://raw.githubusercontent.com/YoungAndSure/TechArt/main/RL/image/rbf_kernel.png)
...
假设m=0,  
$$K = \begin{bmatrix} k(\theta_{1},\theta_{1})  &  k(\theta_{1},\theta_{2}) &  \cdots &  k(\theta_{1},\theta_{k})\\ k(\theta_{2},\theta_{1})  &  k(\theta_{2},\theta_{2}) &  \cdots &  k(\theta_{2},\theta_{k})\\ \vdots &  \vdots &  \ddots &  \vdots \\ k(\theta_{k},\theta_{1})  &  k(\theta_{k},\theta_{2}) &  \cdots &  k(\theta_{k},\theta_{k}) \end{bmatrix}$$
也就是
...
$$\begin{bmatrix} r(\theta_1) \\ f(\theta_*) \end{bmatrix} \sim \mathcal{N}\left( \begin{bmatrix} 0 \\ 0 \end{bmatrix}, \begin{bmatrix} k_{11} + \sigma_n^2 & k_{1*} \\ k_{*1} & k_{**} \end{bmatrix} \right)$$
其中：  
...
同时考虑了收益均值和不确定性，通过参数$\beta$可以控制不确定性的占比。  
如$\beta=0$，则完全不考虑不确定性，就选收益均值最高的点尝试，比较冒险。  
如$\beta=1$，则不确定性也在考虑内，收益均值相同的两个点，优先选不确定性高的，因为不确定性高的点样本量少，更值得冒险。不确定性相同的两个点，优先选择收益均值高的尝试。  

### 问题
- 先验全靠人工对黑盒函数$f$的理解去构造，完全人工对模型的假设，如果假设和实际偏差大，则效果差。
- 随着参数维度增加，点之间的距离变远，效果会下降。  
  
以上介绍的这种方法叫高斯过程贝叶斯优化GP-BO，只是贝叶斯优化的一种，还有很多其他改良版，后面有机会再探索。  
