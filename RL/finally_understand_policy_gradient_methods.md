...
$$E_\tau[G(\tau)] = \sum_{\tau} \operatorname{Pr}(\tau) G(\tau)$$
...
将 $\text{Pr}(\tau \mid \theta)$ 代入 $E_\tau[G(\tau)] = \sum_{\tau} \operatorname{Pr}(\tau \mid \theta) G(\tau)$:

$$
\begin{aligned}
E_\tau[G(\tau)] &= \sum_\tau \operatorname{Pr}(\tau \mid \theta) G(\tau) \\
...
\begin{aligned}
J(\theta) &= E_\tau[G(\tau)] \\
&= \sum_\tau \operatorname{Pr}(\tau) G(\tau) \\
&= \sum_\tau \left[ p(S_0) \prod_{t=0}^T \pi_\theta(A_t \mid S_t) p(S_{t+1} \mid S_t, A_t) \right] G(\tau) \\
&= \sum_{S_0, A_0, S_1, \ldots, A_T, S_{T+1}} \left[ p(S_0) \prod_{t=0}^T \pi_\theta(A_t \mid S_t) p(S_{t+1} \mid S_t, A_t) \right] \times \sum_{t=0}^T \gamma^t R_t
\end{aligned}
$$
...
& = \nabla_\theta \sum_\tau \operatorname{Pr}(\tau \mid \theta) G(\tau) \\
& = \sum_\tau \nabla_\theta \left( \operatorname{Pr}(\tau \mid \theta) G(\tau) \right) \\
& = \sum_\tau \left\{ G(\tau) \nabla_\theta \operatorname{Pr}(\tau \mid \theta) + \operatorname{Pr}(\tau \mid \theta) \nabla_\theta G(\tau) \right\} \\
& = \sum_\tau G(\tau) \nabla_\theta \operatorname{Pr}(\tau \mid \theta)
\end{align*}
$$
...
这里的变形有点意思，涉及另外一个证明：
$$\nabla_\theta \log \operatorname{Pr}(\tau \mid \theta) = \frac{\nabla_\theta \operatorname{Pr}(\tau \mid \theta)}{\operatorname{Pr}(\tau \mid \theta)}$$

证明：设 $u = \text{Pr}(\tau \mid \theta)$，则：
$$\frac{d}{d\theta} \log u = \frac{1}{u} \cdot \frac{du}{d\theta}$$

带回后：
$$\nabla_\theta \log \operatorname{Pr}(\tau \mid \theta) = \frac{1}{\operatorname{Pr}(\tau \mid \theta)} \cdot \nabla_\theta \operatorname{Pr}(\tau \mid \theta)$$
...
\nabla_\theta J(\theta) & = \nabla_\theta \mathbb{E}_{\tau \sim \pi_\theta}[G(\tau)] \\
& = \sum_\tau G(\tau) \text{Pr}(\tau \mid \theta) \nabla_\theta \log \text{Pr}(\tau \mid \theta) \\
& = \mathbb{E}_{\tau \sim \pi_\theta} \left[ G(\tau) \nabla_\theta \log \operatorname{Pr}(\tau \mid \theta) \right]
\end{align*}
$$

发现没有，这里把期望的梯度，换成了梯度的期望。为什么要这么做呢？因为只有换成梯度的期望，才能应用蒙特卡洛法，用采样近似。也就是说，我们可以在多回合游戏中，通过计算 $G(\tau) \nabla_\theta \log \operatorname{Pr}(\tau \mid \theta)$ 取平均值，来近似 $\mathbb{E}_{\tau \sim \pi_\theta} \left[ G(\tau) \nabla_\theta \log \operatorname{Pr}(\tau \mid \theta) \right]$，而不用去求 $\text{Pr}(\tau \mid \theta)$。上面已经说过了，由于状态转移概率无法获取到，所以 $\text{Pr}(\tau \mid \theta)$ 是获取不到的。

继续求 $\nabla_\theta \log \operatorname{Pr}(\tau \mid \theta)$：
$$
\begin{align*}
\operatorname{Pr}(\tau \mid \theta) &= p(S_0) \pi_\theta(A_0 \mid S_0) p(S_1 \mid S_0, A_0) \cdots \pi_\theta(A_T \mid S_T) p(S_{T+1} \mid S_T, A_T) \\
&= p(S_0) \prod_{t=0}^T \pi_\theta(A_t \mid S_t) p(S_{t+1} \mid S_t, A_t)
\end{align*}
$$
...
$$
\begin{align*}
\log \operatorname{Pr}(\tau \mid \theta) &= \log p(S_0) + \sum_{t=0}^T \log p(S_{t+1} \mid S_t, A_t) + \sum_{t=0}^T \log \pi_\theta(A_t \mid S_t)
\end{align*}
$$
