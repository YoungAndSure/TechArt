...
>问了下deepseek这里证明的方法是什么方法，deepseek:"证明本质是应用巴拿赫不动点定理:若一个算子 T 在完备度量空间上是压缩映射​（即存在 $`\gamma<1`$ 满足$`||T(v)−T(w)||≤\gamma||v−w||）`$，则其不动点存在唯一，且任意初始值迭代$`v_{k+1}=T(v_k)`$ 均收敛到该不动点。"不太理解，先记下了。